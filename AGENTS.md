...
Vexel Health Passport (`com.vexel.passport`) is a multi-module Kotlin/Compose Gradle project, not just a spec pack. `app/` owns navigation/DI and currently contains all real screen implementations plus the shared `PassportViewModel` (`app/src/main/kotlin/com/vexel/passport/VexelHealthPassportApp.kt`); `core/{common,database,datastore,designsystem,domain,files,model,notifications,security,testing,ui}/` hold shared infrastructure; `feature/{onboarding,dashboard,profile,timeline,symptoms,records,medications,reminders,appointments,reports,settings}/` are placeholder scaffolding (5-line stub files) reserved for a future modularization pass — see decision log D-008 — not yet real, independent feature modules. `docs/` is the canonical documentation set (see `docs/README.md` for the authority order); the original numbered spec pack is preserved verbatim under `docs/archive/original-ai-development-pack/` and is no longer the source of truth. Keep tests beside their modules in `src/test` and `src/androidTest`, and keep private health data out of the repository.
